...
sub getArmEvsLink
{
	...
	if($idevsUtilLink eq ""){
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsQnapArmBin'}{'zip'};
	...
	elsif($idevsUtilLink eq $EvsQnapArmBin)
	{
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsSynoArmBin'}{'zip'};
	...
	elsif($idevsUtilLink eq $EvsSynoArmBin)
	{
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsNetgArmBin'}{'zip'};
	...
	elsif($idevsUtilLink eq $EvsNetgArmBin)
	{
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsUnvBin'}{'zip'};
...
sub get32bitEvsLink {
	if($idevsUtilLink eq ""){
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsBin32'}{'zip'};
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsQnapBin32_64'}{'zip'};
		$prevPathIdev = $archiveNames{'EvsQnapBin32_64'}{'folder'};
	}
	elsif($idevsUtilLink eq $EvsQnapBin32_64){
		#try with synology 32 evs binary
		$idevsUtilLink = $EvsSynoBin32_64;
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsSynoBin32_64'}{'zip'};
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsNetgBin32_64'}{'zip'};
...
sub get64bitEvsLink 
{
	if($idevsUtilLink eq ""){
		...
		$idevsZip = $idriveSerivePath."/".$archiveNames{'EvsBin64'}{'zip'};
...
sub formWgetCmd {
	...
	if(${$_[0]} eq 1) {
		...
		$wgetCmd = "$wget \"--no-check-certificate\" \"--directory-prefix=$idriveSerivePath\" \"--tries=2\" -e \"http_proxy = $proxy\" $copyIdevsUtilLink \"--output-file=$idriveSerivePath/traceLog.txt\" ";
	} elsif(${$_[0]} eq 0) {
			$wgetCmd = "$wget \"--no-check-certificate\" \"--output-file=$idriveSerivePath/traceLog.txt\" \"--directory-prefix=$idriveSerivePath\" $copyIdevsUtilLink";
	}
...
sub updateConfFile {
	...
	if ($loggedInStat == 1 and $CurrentUser ne '' and  $sameUser==0){
		...
		if ((-e $ManualBackupPidpath) or (-e $ManualRestorePidpath)){
...
sub validateAccountUsingEvs{
	$backupHost = `hostname`;
	chomp($backupHost);
	my $accountVerifyUtfFile = getOperationFile(Constants->CONST->{'ValidateOp'});
	...
	if (!(-e $serviceFileLocation)){
		...
		if($userServicePath eq ''){
			...
			if ($usrInputSerPathExists == 0 and $userServicePathExists == 1){
				...
				print $lineFeed.Constants->CONST->{'displayUserMessage'}->(Constants->CONST->{'ServiceDirectory'},"\"$usrInputSerPath\"",$retErr.'. '.Constants->CONST->{'changeServicePath'});
				my $choice = getConfirmationChoice();
				Chomp($choice);
				if ($choice =~/^n$/i){
					print $lineFeed.Constants->CONST->{'displayUserMessage'}->(Constants->CONST->{'RecreateServiceDir'},"\"$usrInputSerPath\"","and",lc(substr(Constants->CONST->{'TryAgain'},7))).$lineFeed;
