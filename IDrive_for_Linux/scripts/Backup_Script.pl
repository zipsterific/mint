...
if ($flagToCheckSchdule == 0 and $silentBackupFlag == 0){
	if ($dedup eq 'off'){
		...
		print qq{Your Backup Location name is "}.(split('#',$backupHost))[1].qq{". $lineFeed};
...
sub loadFullExclude {
	...
	push @excludeArray, $idriveSerivePath;
...
{ 
	...
	$backupUtfFile = getOperationFile(Constants->CONST->{'BackupOp'}, $parameter_list[0] ,$parameter_list[1] ,$parameter_list[2], $encType);
