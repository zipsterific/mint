	...
	while (1) {	
		...
		for(my $cnt = 0; $cnt < $bufIndex; $cnt++) {
			...
			if($total_fields >= $fields_in_progress) {
				...
				if($totalSize ne Constants->CONST->{'CalCulate'}){
					if($flagToCheckSchdule && $fileOpenflag) {
						$progress = "$backupType Backup"."|Idrive|".$fileSize."|Idrive|".$IncFileSizeByte."|Idrive|".$totalSize."|Idrive|".$fields[($fields_in_progress-$trfPos)]."|Idrive|".$fullHeader."|Idrive|".$pKeyString;
						$newLen = length($progress);
						if($newLen < $prevLen) {
							$pSpace = " "x($prevLen-$newLen);
							$progress = $progress.$pSpace;
						...
						displayProgressBar("$backupType Backup", $fileSize, $IncFileSizeByte, $totalSize, $fields[($fields_in_progress-$trfPos)], $fullHeader, $pKeyString, $curLines);
					}
				}
			}
			elsif($tmpLine =~ /$termData/ ) {
