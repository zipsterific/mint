...
=begin		open ERROR_FD, "< $idevsErrorFile" or traceLog(Constants->CONST->{'FileOpnErr'}." $idevsErrorFile. Reason: $!\n", __FILE__, __LINE__);
		my @errorStr = <ERROR_FD>;
		$errStr = join (' ',@errorStr);
		Chomp(\$errStr);
		if($#errorStr>-1){
			$exit_flag = "1-$errStr";
			chomp($exit_flag);
...
sub doRestoreOperation()
{
	...
	$restoreUtfFile = getOperationFile(Constants->CONST->{'RestoreOp'}, $parameter_list[0] ,$parameter_list[1] ,$parameter_list[2], $encType);
...
#Below section has been added to provide user friendly message and clear instruction in case of password mismatch or encryption verification failed. In this case we are removing the IDPWD file.So that user can login and recreate these files with valid credential.	
				...
				} elsif($errStr =~ /failed to get the device information/i){
