...
#our evsWebPath = "www.${appTypeSupport}.com/downloads/linux/download-options/${appType}_%s";
our $evsWebPath = "www.${appTypeSupport}downloads.com/downloads/linux/download-options/";
#if($appType eq "IDrive") {
	our $EvsBin32 = $evsWebPath.$archiveNames{'EvsBin32'}{'zip'};
	#our $EvsBin32 = sprintf($evsWebPath,"linux_32bit.zip");
	our $EvsBin64 = $evsWebPath.$archiveNames{'EvsBin64'}{'zip'};
	#our $EvsBin64 = sprintf($evsWebPath,"linux_64bit.zip");
	our $EvsQnapBin32_64 = $evsWebPath.$archiveNames{'EvsQnapBin32_64'}{'zip'};
	#our $EvsQnapBin32_64 = sprintf($evsWebPath,"QNAP_64bit.zip");
	our $EvsSynoBin32_64 = $evsWebPath.$archiveNames{'EvsSynoBin32_64'}{'zip'};
	#our $EvsSynoBin32_64 = sprintf($evsWebPath,"synology_64bit.zip");
	our $EvsNetgBin32_64 = $evsWebPath.$archiveNames{'EvsNetgBin32_64'}{'zip'};
	#our $EvsNetgBin32_64 = sprintf($evsWebPath,"Netgear_64bit.zip");
	our $EvsUnvBin = $evsWebPath.$archiveNames{'EvsUnvBin'}{'zip'};
	#our $EvsUnvBin = sprintf($evsWebPath,"linux_universal.zip");
	our $EvsQnapArmBin = $evsWebPath.$archiveNames{'EvsQnapArmBin'}{'zip'};
	#our $EvsQnapArmBin = sprintf($evsWebPath,"QNAP_ARM.zip");
	our $EvsSynoArmBin = $evsWebPath.$archiveNames{'EvsSynoArmBin'}{'zip'};
	#our $EvsSynoArmBin = sprintf($evsWebPath,"synology_ARM.zip");
	our $EvsNetgArmBin = $evsWebPath.$archiveNames{'EvsNetgArmBin'}{'zip'};
	#our $EvsNetgArmBin = sprintf($evsWebPath,"Netgear_ARM.zip");
...
our $IBackupAccVrfLink = "https://www1.ibackup.com/cgi-bin/get_ibwin_evs_details_xml_ip.cgi?";
our $IDriveUserInoCGI  =  "https://www1.idrive.com/cgi-bin/update_user_device_info.cgi?";
#*******************************************************************************************************/
...
sub getOperationFile
{
	...
	$_[1] = defined($_[1]) ? $_[1] : checkEncType(1);
	...
	if($dedup eq 'on'){
		$deviceIDParam   = $hashEvsParameters{DEVICEID}.$assignmentOperator.$deviceID.$lineFeed;
		#$itemStatusParam = $hashEvsParameters{ITEMSTATUS3}.$lineFeed;
	...
	if ($operationType eq Constants->CONST->{'LinkBucketOp'}){
		$utfPath = defined ($usrProfileDir) ? $usrProfileDir."/utf.txt" : $usrProfilePath.'/utf.txt';
                ...
                $hashEvsParameters{NICKNAME}.$assignmentOperator.$deviceNickName.$lineFeed.
		...
		$serverName.$pathSeparator.$lineFeed.
                $hashEvsParameters{DEVICEID}.$assignmentOperator.$deviceID.$lineFeed.
		...
		$utfFile .= $hashEvsParameters{PROXY}.$assignmentOperator.$proxyStr.$lineFeed.
					...
					$hashEvsParameters{ERROR}.$assignmentOperator.$idevsErrorFile.$lineFeed.
					$hashEvsParameters{ADDPROGRESS}.$lineFeed.$xmlOutputParam.$deviceIDParam.
					$source.$lineFeed.
					$userName.$serverAddressOperator.
					$serverAddress.$serverNameOperator.
...
=cut
		...
		$utfFile =	$hashEvsParameters{PASSWORD}.$assignmentOperator.$pwdPath.$lineFeed.
					$hashEvsParameters{PROPERTIES}.$lineFeed.$deviceIDParam.
					$hashEvsParameters{PROXY}.$assignmentOperator.$proxyStr.$lineFeed.
					$hashEvsParameters{ENCODE}.$lineFeed;
		if(!defined($_[1]) && $_[1] ne "modProxy") {
			$utfFile .=	$hashEvsParameters{ERROR}.$assignmentOperator.$idevsErrorFile.$lineFeed;
		...
		$utfFile .= $userName.$serverAddressOperator.
			...
			$utfFile = $hashEvsParameters{PASSWORD}.$assignmentOperator.$pwdPath.$lineFeed.
				   ...
				   $hashEvsParameters{PROXY}.$assignmentOperator.$proxyStr.$lineFeed.
			   	   $hashEvsParameters{ENCODE}.$lineFeed.
				   $hashEvsParameters{ERROR}.$assignmentOperator.$idevsErrorFile.$lineFeed.$deviceIDParam.
			   	   $userName.$serverAddressOperator.
			   	   $serverAddress.$serverNameOperator.
	...
	elsif($operationType eq Constants->CONST->{'GetServerAddressOp'})
	{
		...
		$hashEvsParameters{FILE}.$lineFeed.$xmlOutputParam.$deviceIDParam.
		$userName.$serverAddressOperator.
		$serverAddress.$serverNameOperator.
		...
		$hashEvsParameters{PASSWORD}.$assignmentOperator.$pwdPath.$lineFeed.$deviceIDParam.
		$hashEvsParameters{PROXY}.$assignmentOperator.$proxyStr.$lineFeed.
		$hashEvsParameters{TEMP}.$assignmentOperator.$evsTempDir.$lineFeed.
...
sub parseXMLOutput
{
	...
	if(defined ${$_[0]} and ${$_[0]} ne "") {
		...
		my @evsArrLine = ();
		if ($parseDeviceList){
				@evsArrLine = grep {/\w+/} grep {/bucket_type=\"D\"|No devices found/} split(/(?:\<item|\<login|<tree)/g, $evsOutput);
		}else{
...
sub displayProgressBar
{
	...
	if($jobType =~ /Backup/i) {
		if($dedup eq 'on'){
			my $backupLoaction = (split('#',$backupHost))[1];
...
sub writeLogHeader {
	...
	if($jobType eq "Restore") {
		my $fromLocation = ($restoreHost =~ /#/)?(split('#',$restoreHost))[1]:$restoreHost;
		$mailHeadB .= "$jobType From Location: $fromLocation $lineFeed";
	}	
...
sub createUserDir {
	...
	my @dirArr = ($usrProfilePath, $usrProfileDir, $usrBackupDir, $usrBackupManualDir, $usrRestoreDir, $usrRestoreManualDir,$usrBackupManualDirSch,$usrRestoreManualDirSch);
...
#							unlink($pwdPath);
		...
		if ($dedup eq 'on'){
			($deviceID,$restoreHost) = split ('#',$restoreHost);
		}
		$choice = 'y';
		...
		if($choice eq 'y' || $choice eq 'Y') {
			...
			if (runningJobHandler('Restore','Scheduled',$userName,$usrProfilePath)){
				if ($dedup eq 'off'){
					...
					$userChoice -= 1;
					#use Data::Dumper;
					#print Dumper(\@devicesToLink);
					#print "\nDEBUG :userChoice - $userChoice\n";
					#print Dumper($devicesToLink[$userChoice]);
...
sub getQuota{
	my $encType = checkEncType(1);
	my $getQuotaUtfFile = getOperationFile(Constants->CONST->{'GetQuotaOp'},$encType);
	$getQuotaUtfFile =~ s/\'/\'\\''/g;
...
sub getDeviceList{
	...
	$encType = defined ($_[0]) ? $_[0] : $encType;
        my $listDeviceUtfFile = getOperationFile(Constants->CONST->{'ListDeviceOp'},$encType);
...
sub getUserMenuChoice{
	...
	$userMessage = defined($userMessage) ? $userMessage : Constants->CONST->{'EnterChoice'};
        my $userChoice = 0;
        while ($retryCount){
                print $userMessage;
                $userChoice = <STDIN>;
				...
				if ($userMessage =~ /Press Enter to go back to main menu/ and $userChoice eq ''){
					createOrLinkBucket(--$repeatDisplayBlock);
...
sub linkBucket {
	...
	($deviceID,$deviceNickName) = ($deviceIdPrefix.$linkToData->{device_id}.$deviceIdPostfix,$linkToData->{nick_name});
	my $linkBucketUtfFile = getOperationFile(Constants->CONST->{'LinkBucketOp'},$encType);
	...
	if ($commandOutput =~ /status=["]success["]/i){
			$' =~ /device_id="(.*?)".*?nick_name="(.*?)".*/;
			if ($linkForRestore eq 'restore'){
					$deviceID   = $deviceIdPrefix.$1.$deviceIdPostfix;
					$restoreHost =  "$deviceID#$2";
			}else{
					$deviceID   = $deviceIdPrefix.$1.$deviceIdPostfix;
					$backupHost = $restoreHost =  "$deviceID#$2";
					# BackupLocation = "DeviceID#Nickname" Eg: "D01500371120000812023#/dedup1
					print Constants->CONST->{'BackupLocMsg'}.$whiteSpace."\"$2\"".$lineFeed;
			}
	}
}
#*********************************************************************************************************
#Subroutine Name        : getUniqueID
#Objective              : This function will retrun unique id.
#Usage                  : getUniqueID();
#Added By               : Abhishek Verma.
#*********************************************************************************************************/
sub getUniqueID{
	my $getLinuxFlavour = `cat /etc/*-release`;
	my $ethernetAddrIndex = -1;
	$ethernetAddrIndex = 2 if($getLinuxFlavour =~ /fedora/i);
	$uidRes = `/sbin/ifconfig | grep Ethernet |sed -n 1p|sed s/://g`;
...
sub checkIfEvsWorking {
	my $dedupStatus = shift;
	$idevsutilBinaryPath = "$idriveSerivePath/idevsutil";
	$idevsutilBinaryPath = "$idriveSerivePath/idevsutil_dedup" if ($dedupStatus eq 'on');#here binary path is created for dedup command.
...
sub checkDeviceID{
	my $userInput = $_[0];
	my %evsDeviceHashOutput = getDeviceList();
	return 0 if ($dataKey =~/\@ERROR\s*:\s*.*?/i);
...
sub createOrLinkBucket{
	my $retryCount = $#_ == -1 ? 4 : $_[0];
		...
		my %evsDeviceHashOutput = getDeviceList();
		print Constants->CONST->{'ListOfDevice'}.$lineFeed;
				my @devicesToLink = displayDeviceList(\%evsDeviceHashOutput,\@columnNames);
				print $lineFeed;
		if ($retryCount <= 0){
			print Constants->CONST->{'maxRetry'}.$lineFeed;
...
sub createBucket{
	$encType = checkEncType(1) if($encType eq "");
	my $createBucketUtfFile = getOperationFile(Constants->CONST->{'CreateBucketOp'},$encType);
	...
	if ($commandOutput =~ /status=["]success["]/i){
			$' =~ /device_id="(.*?)".*?nick_name="(.*?)".*/;
			$deviceID   = $deviceIdPrefix.$1.$deviceIdPostfix;
			$backupHost = $restoreHost =  "$deviceID#$2"; # BackupLocation = "DeviceID#Nickname" Eg: "D01500371120000812023#/dedup1 Constants->CONST->{'BackupLocMsg'}.$whiteSpace."\"$2\"".$lineFeed;			
			print Constants->CONST->{'BackupLocMsg'}.$whiteSpace."\"$2\"".$lineFeed;
...
sub nickUpdate {
	$encType = checkEncType(1) if($encType eq "");
	my $nickUpdateUtfFile = getOperationFile(Constants->CONST->{'NickUpdateOp'},$encType);
	...
	if ($commandOutput =~ /status=["]success["]/i){
			$' =~ /nick_name=["](.*?)["].*/;
			if ($locationToEdit eq 'backup'){
					$backupHost = "$deviceID#$1";
			}elsif($locationToEdit eq 'restore'){
					$restoreHost =  "$deviceID#$1";
			}else{
					$backupHost = $restoreHost =  "$deviceID#$1"; # BackupLocation = "DeviceID#Nickname" Eg: "D01500371120000812023#/dedup1 Constants->CONST->{'BackupLocMsg'}.$whiteSpace."\"$2\"".$lineFeed
...
#*********************************************************************************************************
#Subroutine Name        : backupLocationQuery
#Objective              : This function will ask if user wants to rename the backup location/ create new /link to existing backup location/ bucket.
#Usage                  : backupLocationQuery();
#Added By               : Abhishek Verma.
#*********************************************************************************************************/
sub backupLocationQuery{
	my $locationQuery = qq{Your Backup Location name is "}.(split('#',$backupHost))[1].qq{". Do you want to edit(y/n)?};
    print $locationQuery;
	$isSameDeviceID = 0;
	$isSameDeviceID = 1 if($backupHost ne "" and $backupHost eq $restoreHost);	
    my $choice = getConfirmationChoice();
	if ($choice =~ /^y$/i){
		unless (checkDeviceID($choice)){
			return 0;
		}
		$backupHost = $deviceID.'#'.$backupHost if($backupHost !~ /#/);
		putParameterValue(\"BACKUPLOCATION", \"$backupHost", $confFilePath);
		if($isSameDeviceID){
			$restoreHost = $backupHost;
			putParameterValue(\"RESTOREFROM",\"$restoreHost",$confFilePath);#This function will update  restore location to conf file. Added do keep the same Nick name if device ids are same.
		}	
	}
}
#****************************************************************************
